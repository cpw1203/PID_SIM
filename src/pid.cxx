...
double PIDController::calc_error()
{
  ...
  int_val = this->error() * this->sample_time; // calculate integral value
  this->sum_integral += int_val;
  dir_val = (this->error() - this->prev_error) / this->sample_time; // calculate derivative
  this->prev_error = this->error(); // set previous error for next run

  // calculate the total Percent ON 
  error_out = (this->kp * this->error()) + (this->ki * this->sum_integral) + (this->kd * dir_val);
